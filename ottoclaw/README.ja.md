...
<h1>OttoClaw: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 OttoClaw は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 OttoClaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
OttoClaw はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/sipeed/ottoclaw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに OttoClaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `OTTOCLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.ottoclaw/config.json` に API キーを設定してください。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、OttoClaw はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で OttoClaw と会話できます
...
> **Docker Compose**: Gateway HTTP サーバーは共有の `127.0.0.1:18790` で Webhook を提供します。ホストからアクセスするには `ottoclaw-gateway` サービスに `ports: ["18790:18790"]` を追加してください。
...
OttoClaw は3種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.ottoclaw/config.json`

### 環境変数

環境変数を使用してデフォルトのパスを上書きできます。これは、ポータブルインストール、コンテナ化されたデプロイメント、または ottoclaw をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `OTTOCLAW_CONFIG` | 設定ファイルへのパスを上書きします。これにより、ottoclaw は他のすべての場所を無視して、指定された `config.json` をロードします。 | `~/.ottoclaw/config.json` |
| `OTTOCLAW_HOME`   | ottoclaw データのルートディレクトリを上書きします。これにより、`workspace` やその他のデータディレクトリのデフォルトの場所が変更されます。          | `~/.ottoclaw`             |

**例：**

```bash
# 特定の設定ファイルを使用して ottoclaw を実行する
# ワークスペースのパスはその設定ファイル内から読み込まれます
OTTOCLAW_CONFIG=/etc/ottoclaw/production.json ottoclaw gateway

# すべてのデータを /opt/ottoclaw に保存して ottoclaw を実行する
# 設定はデフォルトの ~/.ottoclaw/config.json からロードされます
# ワークスペースは /opt/ottoclaw/workspace に作成されます
...
OttoClaw は設定されたワークスペース（デフォルト: `~/.ottoclaw/workspace`）にデータを保存します：
...
OttoClaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.ottoclaw/workspace` | エージェントの作業ディレクトリ |
...
OttoClaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `OTTOCLAW_HEARTBEAT_ENABLED=false` で無効化
- `OTTOCLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
> **新機能！** OttoClaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
```
> OAuth認証を設定するには、`ottoclaw auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、OttoClaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `ottoclaw onboard` | 設定＆ワークスペースの初期化 |
| `ottoclaw agent -m "..."` | エージェントとチャット |
| `ottoclaw agent` | インタラクティブチャットモード |
| `ottoclaw gateway` | ゲートウェイを起動 |
| `ottoclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。OttoClaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`ottoclaw gateway` が 1 つだけ実行されていることを確認してください。
