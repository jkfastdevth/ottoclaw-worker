...
import (
	...
	configstore "github.com/sipeed/ottoclaw/cmd/ottoclaw-launcher-tui/internal/config"
...
func Run() error {
	...
	if cfg == nil {
		cfg = ottoclawconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open ottoclaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("ottoclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("ottoclaw", "gateway")
