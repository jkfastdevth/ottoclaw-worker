...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					ottoclawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model ottoclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []ottoclawconfig.ModelConfig) {
...
func isModelValid(model ottoclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *ottoclawconfig.ModelConfig) {
	if model == nil {
		return
	}
