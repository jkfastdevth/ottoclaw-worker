...
A standalone launcher for OttoClaw, providing visual JSON editing and OAuth provider authentication management.
...
```bash
# Build
go build -o ottoclaw-launcher ./cmd/ottoclaw-launcher/

# Run with default config path (~/.ottoclaw/config.json)
./ottoclaw-launcher

# Specify a config file
./ottoclaw-launcher ./config.json

# Allow LAN access
./ottoclaw-launcher -public
```
...
```
Usage: ottoclaw-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.ottoclaw/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.ottoclaw/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the ottoclaw-config UI.
...
Gets the running status of the `ottoclaw gateway` process.
...
Starts the `ottoclaw gateway` process in the background.
...
Stops the running `ottoclaw gateway` process.
...
```bash
go test -v ./cmd/ottoclaw-launcher/
```
