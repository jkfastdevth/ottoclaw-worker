...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-ottoclaw-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".ottoclaw", "config.json")) {
		t.Errorf("expected path ending with .ottoclaw/config.json, got %q", path)
	}
}
