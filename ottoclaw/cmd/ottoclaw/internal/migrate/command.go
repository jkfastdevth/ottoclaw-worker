...
import (
	...
	"github.com/sipeed/ottoclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to ottoclaw",
		Args:  cobra.NoArgs,
		Example: `  ottoclaw migrate
  ottoclaw migrate --from openclaw
  ottoclaw migrate --dry-run
  ottoclaw migrate --refresh
  ottoclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.ottoclaw)")
