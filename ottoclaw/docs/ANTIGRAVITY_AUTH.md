...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in OttoClaw.
...
10. [Creating a New Provider in OttoClaw](#creating-a-new-provider-in-ottoclaw)
...
const antigravityPlugin = {
  ...
  register(api: OttoClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<OttoClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- OttoClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.ottoclaw/auth.json`:
...
## Creating a New Provider in OttoClaw

OttoClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/ottoclaw/cmd_auth.go`:
...
ottoclaw auth login --provider your-provider
...
ottoclaw agent -m "Hello" --model your-model
```
...
export OTTOCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export OTTOCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `ottoclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.ottoclaw/auth.json`
- Re-run `ottoclaw auth login --provider antigravity`
