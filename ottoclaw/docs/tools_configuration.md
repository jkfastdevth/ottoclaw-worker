...
OttoClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, OttoClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `OTTOCLAW_TOOLS_<SECTION>_<KEY>`:

For example:

- `OTTOCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `OTTOCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `OTTOCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `OTTOCLAW_TOOLS_MCP_ENABLED=true`
