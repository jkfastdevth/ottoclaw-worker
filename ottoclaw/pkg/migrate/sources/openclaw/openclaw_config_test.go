...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.ottoclaw/workspace" {
		t.Errorf("expected workspace '~/.ottoclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToOttoClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToOttoClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.ottoclaw/workspace" {
		t.Errorf("expected workspace '~/.ottoclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToOttoClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToOttoClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.ottoclaw/workspace" {
		t.Errorf("expected workspace '~/.ottoclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
