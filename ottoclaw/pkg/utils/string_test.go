...
func TestStripInternalTags(t *testing.T) {
	tests := []struct {
		...
		{"only function classic", "hi <function>msg{...}</function>", "hi"},
		{"only function call end", `hi <function>message{"key":"val"}</FunctionCallEnd>`, "hi"},
		{"function call end with spaces", `<function>send{"content": "hello"}</FunctionCallEnd> extra`, "extra"},
