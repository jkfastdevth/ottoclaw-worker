...
func (t *FacebookTool) post(ctx context.Context, message string) *ToolResult {
	...
	steps := []map[string]any{
		...
		{
			"action":     "wait_for",
			"selector":   "[aria-label='สร้างโพสต์'],[aria-label='Create post'],[data-testid='status-attachment-mentions-input'],span:text-is(\"คุณคิดอะไรอยู่\"),span:text-is(\"What's on your mind?\")",
			"timeout_ms": 20000,
		},
		...
		{
			"action":   "click",
			"selector": "[aria-label='สร้างโพสต์'],[aria-label='Create post'],span:text-is(\"คุณคิดอะไรอยู่\"),span:text-is(\"What's on your mind?\")",
		},
