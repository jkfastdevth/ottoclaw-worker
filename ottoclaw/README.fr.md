<div align="center">
  ...
  <h1>OttoClaw : Assistant IA Ultra-Efficace en Go</h1>
...
🦐 **OttoClaw** est un assistant personnel IA ultra-léger inspiré de [nanobot](https://github.com/HKUDS/nanobot), entièrement réécrit en **Go** via un processus d'auto-amorçage (self-bootstrapping) — où l'agent IA lui-même a piloté l'intégralité de la migration architecturale et de l'optimisation du code.
...
>
> * **PAS DE CRYPTO :** OttoClaw n'a **AUCUN** token/jeton officiel. Toute annonce sur `pump.fun` ou d'autres plateformes de trading est une **ARNAQUE**.
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[ottoclaw.io](https://ottoclaw.io)**, et le site de l'entreprise est **[sipeed.com](https://sipeed.com)**.
> * **Attention :** De nombreux domaines `.ai/.org/.com/.net/...` sont enregistrés par des tiers et ne nous appartiennent pas.
> * **Attention :** OttoClaw est en phase de développement précoce et peut présenter des problèmes de sécurité réseau non résolus. Ne déployez pas en environnement de production avant la version v1.0.
> * **Note :** OttoClaw a récemment fusionné de nombreuses PR, ce qui peut entraîner une empreinte mémoire plus importante (10–20 Mo) dans les dernières versions. Nous prévoyons de prioriser l'optimisation des ressources dès que l'ensemble des fonctionnalités sera stabilisé.


## 📢 Actualités

2026-02-16 🎉 OttoClaw a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! OttoClaw grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 OttoClaw a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de OttoClaw.
...
2026-02-09 🎉 OttoClaw est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 OttoClaw, c'est parti !
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec OttoClaw. Démarrage rapide :
...
OttoClaw peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Téléchargez le binaire pour votre plateforme depuis la page des [releases](https://github.com/sipeed/ottoclaw/releases).
...
Vous pouvez également exécuter OttoClaw avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `OTTOCLAW_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm ottoclaw-agent -m "Combien font 2+2 ?"
...
> [!TIP]
> Configurez votre clé API dans `~/.ottoclaw/config.json`.
...
**2. Configurer** (`~/.ottoclaw/config.json`)
...
> `request_timeout` est optionnel et s'exprime en secondes. S'il est omis ou défini à `<= 0`, OttoClaw utilise le délai d'expiration par défaut (120s).
...
```bash
ottoclaw agent -m "Combien font 2+2 ?"
```
...
Discutez avec votre OttoClaw via Telegram, Discord, DingTalk, LINE ou WeCom
...
OttoClaw prend en charge trois types d'intégration WeCom :
...
Connectez OttoClaw au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.ottoclaw/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de ottoclaw en tant que service système. Ces variables sont indépendantes et contrôlent différents chemins.

| Variable          | Description                                                                                                                             | Chemin par Défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `OTTOCLAW_CONFIG` | Remplace le chemin du fichier de configuration. Cela indique directement à ottoclaw quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.ottoclaw/config.json` |
| `OTTOCLAW_HOME`   | Remplace le répertoire racine des données ottoclaw. Cela modifie l'emplacement par défaut du `workspace` et des autres répertoires de données.          | `~/.ottoclaw`             |

**Exemples :**

```bash
# Exécuter ottoclaw en utilisant un fichier de configuration spécifique
...
# Exécuter ottoclaw avec toutes ses données stockées dans /opt/ottoclaw
# La configuration sera chargée à partir du fichier par défaut ~/.ottoclaw/config.json
# Le workspace sera créé dans /opt/ottoclaw/workspace
...
OttoClaw stocke les données dans votre workspace configuré (par défaut : `~/.ottoclaw/workspace`) :
...
OttoClaw s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.ottoclaw/workspace` | Répertoire de travail de l'agent |
...
OttoClaw peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `OTTOCLAW_HEARTBEAT_ENABLED=false` pour désactiver
* `OTTOCLAW_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
```bash
ottoclaw agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** OttoClaw utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
```
> Exécutez `ottoclaw auth login --provider anthropic` pour configurer les identifiants OAuth.
...
Configurez plusieurs points de terminaison pour le même nom de modèle—OttoClaw utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `ottoclaw onboard`        | Initialiser la configuration & le workspace |
| `ottoclaw agent -m "..."` | Discuter avec l'agent                 |
| `ottoclaw agent`          | Mode de discussion interactif         |
| `ottoclaw gateway`        | Démarrer la passerelle                |
| `ottoclaw status`         | Afficher le statut                    |
| `ottoclaw cron list`      | Lister toutes les tâches planifiées   |
| `ottoclaw cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

OttoClaw prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.ottoclaw/workspace/cron/` et traitées automatiquement.
...
C'est normal si vous n'avez pas encore configuré de clé API de recherche. OttoClaw fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.ottoclaw/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `ottoclaw gateway` fonctionne à la fois.
