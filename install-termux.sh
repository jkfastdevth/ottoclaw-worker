...
generate_config_json() {
    ...
    if [[ -n "${WORKER_TELEGRAM_TOKEN:-}" ]]; then
        ...
        [[ "${TELEGRAM_ORCHESTRATION_ENABLED:-false}" == "true" ]] && ORCH_STATE="true"
            local _bridge; _bridge=$(echo "${TELEGRAM_BRIDGE_CHAT_ID}" | sed 's/["\\]/\\&/g')
            ORCH_FRAG=", \"orchestration_enabled\": ${ORCH_STATE}, \"bridge_chat_id\": \"${_bridge}\""
        fi
        local _tok; _tok=$(echo "${WORKER_TELEGRAM_TOKEN}" | sed 's/["\\]/\\&/g')
        TG_FRAG=", \"telegram\": { \"enabled\": true, \"token\": \"${_tok}\"${ALLOW_FRAG}${ORCH_FRAG}, \"typing\": {\"enabled\": true} }"
    fi
